...
import {useAdminDataStore} from "../../../../stores/AdminDataStore.js"; // Import Bootstrap's modal


const OrganizationDetailEditPracticeModal = () => {
    const {authToken} = useContext(AuthContext);
    const navigate = useNavigate();
    const { loginUser } = useAdminDataStore();
    const { organizationData,organizationPractice,setOrganizationReload } = useOrganizationDataStore();
    const [formValues, setFormValues] = useState({
        OrganizationPracticeID: '',
        OrganizationID: '',
        PracticeName: '',
        Street: '',
        City: '',
    ...
    useEffect(() => {
        if (organizationPractice && organizationPractice.OrganizationPracticeID) {
            setFormValues({
                OrganizationPracticeID: organizationPractice.OrganizationPracticeID || '',
                OrganizationID: organizationPractice.OrganizationID || '',
                PracticeName: organizationPractice.PracticeName || '',
                Street: organizationPractice.Street || '',
                City: organizationPractice.City || '',
                State: organizationPractice.State || '',
                ZipCode: organizationPractice.ZipCode || ''
            });
        }
    }, [organizationPractice, authToken]);
    ...
    return (
        <div className="organization-detail-modal modal fade"
             id="organization-detail-edit-practice-modal" tabIndex={-1}>
            <div className="modal-dialog modal-dialog-centered">
                <form className="w-100" onSubmit={handleSubmit}>
                    <div className="modal-content">
                        <div className="modal-header border-bottom-0">
                            <h5 className="modal-title text-dark-grey-4 fw-bolder">Edit Practice</h5>
                        ...
                        <div
                            ...
                            <button type="submit" className="btn btn-primary rounded-2 fs-14 fw-normal">Update</button>
                            <button type="button"
                                    ref={cancelButtonRef}
...
export default OrganizationDetailEditPracticeModal;
