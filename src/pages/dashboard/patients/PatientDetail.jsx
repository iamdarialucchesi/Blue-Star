...
function PatientDetail() {
    ...
    const fetchAllPatientDocuments = async (patient_id) => {
        try {
            const result = await PatientsService.fetchPatientPatientDocuments(authToken, patient_id);
    ...
    const handleUploadDocument = async (imageUrl, fileName) => {
        try {
            ...
            const result = await PatientsService.uploadPatientDocument(authToken, formValues);
            setIsLoading(false);
    ...
    const handleDocumentDelete = async (e) => {
        try {
            e.stopPropagation();
            setIsLoading(true);
            const result = await PatientsService.deletePatientDocument(authToken, loginUser, deletedDocumentID);
            setIsLoading(false);
    ...
    return (
        <AdminLayout headerTitle={"Patient Detail"} isDashboard={true}>
            <div className='patient-detail-secreen pt-2'>
                <div className="card mb-3">
                    <div
                        className="card-header py-3 d-flex justify-content-between align-items-center patient-card-background-color">
                        <h4 className='card-heading fw-bold m-0'>{patient.FirstName} {patient.LastName}</h4>
                        <div className='card-icons d-flex gap-2'>
                            <button type="button" className="btn border-0" onClick={(e) => handlePatientEdit(e, patient)}>
                            ...
                            <div className='d-flex justify-content-between border-bottom pt-3'>
                                ...
                                <div>
                                    ...
                                    <ul className="dropdown-menu py-0 px-3 role-accessibility-dropdown-inset cursor-pointer" ref={dropdownRef}>
                                        {patientPrograms.Programs.map(program => (
                                            <li key={program.label} onClick={() => handleProgramChange(program)}>
                        ...
                        <div className='custom-scrollbar-section patient-section-border rounded-3 px-3'>
                            ...
                            <div className='mt-1 detail-text'>
                                {/* <div
                                    ...
                                    Screening <button
                                        className='detail-text btn btn-primary px-2 py-1'>Schedule</button>
                                </div> */}
...
function Medications(medications) {
    return <div className='mt-2'>
        ...
        {medications.medications.map((medication, index) => (
            <p key={index} className='tab-content-background-color content-text-color detail-text  fw-light px-3 py-2 rounded-4 d-flex justify-content-between align-items-center'>
                <span className='w-75'>{medication.Name}</span><span className='w-25'>{medication.Dosage}</span>
            </p>
        ))}
    </div>
}
